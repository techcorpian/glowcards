...
const Card = ({ title, description }) => {
    ...
    return (
        <div
            ...
            className='relative bg-gradient-to-tr from-gray-900 to-white/20 py-4 px-6 rounded-2xl shadow-lg backdrop-filter backdrop-blur-lg w-full h-[300px] cursor-pointer'
            style={{
        ...
        >
            ...
            <div className='flex flex-col gap-4'>
                <div className='flex justify-between items-start'>
                    <h1 className='relative text-white text-2xl'>{title}</h1>
                    <div className='relative text-gray-600 text-2xl hover:text-white'><MdOutlineArrowOutward /></div>
                </div>
                <p className='relative text-gray-500 text-md font-thin'>{description}</p>
            </div>
        </div>
    );
};
